...
`tools/check_critical_coverage.py` запускает 119 safety tests и измеряет branch
coverage четырёх критических runtime-модулей. Нижняя граница равна 75%. На
эталонном запуске суммарное branch coverage равно 75%, отдельные модули дают
70-83%. Любое падение ниже порога блокирует CI и release.
