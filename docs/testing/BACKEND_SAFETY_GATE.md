...
`tools/check_critical_coverage.py` запускает полный локальный набор pytest и измеряет branch
coverage девяти критических runtime-модулей, включая общий ручной приоритет
