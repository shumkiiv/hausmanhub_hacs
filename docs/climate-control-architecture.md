...
Версия документа: 2026-07-18. Реализация относится к HASC 0.5.6.
...
контракты остаются в `v1`; прежняя home schema v1 сохранена для явного
распознавания старого ответа. В 0.5.6 актуальный Android home-контракт находится
в `v2/climate-home.schema.json`. Android должен закрепить поддерживаемую версию
и закрыться при неизвестной версии, а не угадывать форму ответа. Всего
устанавливается тринадцать схем.

Home v2 добавляет к каждой комнате только публичный результат управления:
...
Climate API command types. `enabled=true` возможен только для выбранной canary
...
1. Установить HASC 0.5.6, оставить climate bridge в `disabled`.
...
0.5.6 подготавливает для Android только публичный home v2 со статусом кнопок и
причинами блокировки. Она не меняет Android-репозиторий и не разрешает
