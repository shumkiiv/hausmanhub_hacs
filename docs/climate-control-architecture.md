...
Версия документа: 2026-07-18. Реализация относится к HASC 0.6.0.
...
HASC: комнаты, цели, встроенное решение, публичные действия и проверки
       ↕ два фиксированных маршрута Climate API v1
существующий climate-core / Smart Home Center — переходный источник и исполнитель
       ↕ его текущие адаптеры, cooldown, safety и physical feedback
Home Assistant / Node-RED / физические устройства
```

HASC становится единственной точкой, которую должен знать Android. Начиная с
0.6.0 климатические решения постепенно переходят внутрь HASC. Первый этап уже
хранит цели одной комнаты и рассчитывает: нужен нагрев, охлаждение,
увлажнение или удержание текущего состояния.

Переход выполняется без резкого переключения. Существующий climate-core пока
даёт HASC фактические показания и остаётся физическим исполнителем. Его
cooldown, authority, safety и physical feedback сохраняются, пока аналогичные
нативные адаптеры HASC не будут отдельно реализованы, сравнены и разрешены.
Android при этом всегда работает только с HASC и не зависит от того, какой
исполнитель находится под ним.

## Что делает встроенный контур HASC 0.6.0

В отдельном разделе настроек можно выбрать одну сохранённую комнату и задать:

- температуру от 18 до 28 °C с шагом 0,5 °C;
- влажность от 30 до 70 % с шагом 5 %.

HASC берёт свежие показания из уже подключённого Climate API и применяет
фиксированную нейтральную зону: ±0,5 °C и ±5 % влажности. Результат показывает
потребность комнаты и наличие доступного устройства подходящего типа.
Устаревшие или отсутствующие показания не превращаются в решение.

Этот этап принципиально не исполняет результат: контракт предпросмотра всегда
возвращает `execution.mode=preview_only` и `commands_enabled=false`. В коде
расчёта нет транспорта или вызова сервиса Home Assistant. Если связь с
существующим контуром выключена, предпросмотр не выполняет сетевой запрос и
объясняет, что текущих данных нет.
...
распознавания старых ответов. В 0.6.0 актуальный Android home-контракт находится
...
1. Установить HASC 0.6.0, оставить выполнение климатических команд выключенным.
...
4. Открыть «Встроенный климатический контур HASC», выбрать одну сохранённую
   комнату и задать цели. Проверить показанные HASC текущие значения, решение и
   наличие оборудования. На экране должно быть явно указано, что команды
   запрещены.
5. Проверить Android snapshot и оба начальных shadow-действия. Ни одно из них
   не должно выполнить POST. Подождать три разнесённых наблюдения.
6. В мастере выбрать «Посмотреть shadow-доказательства», затем комнату. Пока
   результат не `ready`, canary остаётся закрыт.
7. Выбрать «Проверить готовность canary одной комнаты». Требовать полный
   preflight `ready`, `operation=clear`, `rollback=ready` и точный scope из
   двух начальных действий. Это ещё не включает управление.
8. Проверить неактивный [one-room rollout
   ...
   room ID.
9. Проверить одно изменение target, затем перечитать snapshot и физическое
   подтверждение в существующем контуре.
10. При любом расхождении вернуть `disabled`: это удаляет адрес и room ID из
   options и немедленно закрывает командный путь.

В репозитории нет live-адреса и готового реестра конкретного дома. Версия
0.6.0 сохраняет для Android публичный home v4 со статусом кнопок,
...
физический canary. Дополнительно она вводит локальный предпросмотр решения
HASC, но не подключает его к пути исполнения. Переключение установленного дома
