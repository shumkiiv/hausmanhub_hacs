...
Checkpoint частей 3/4: `4ea978b`, точный commit проверен исполнителем:
2640 passed, 4 skipped, 1238 subtests за 309.79s; browser 47 passed,
16 состояний/735 элементов без ошибок. Package PASS после commit.
Migration regression 36/36 включает настоящий ремонт повторного staging
двух replacement, exact hashes/key sets/topology и независимый before.
Физическое подтверждение питания/оттенков и общая приёмка ещё впереди.
Версия 1.52.226 требует повышения перед выпуском, activation_ready закрыт.
Следующая реализация: часть 5. Ниже критерии сохраняются для tester.
...
- [ ] Шкала физического полного хода пока не подтверждена. Сохранить
  проверку current_position>20 перед close_cover кабинета до визуальной
  калибровки root с владельцем. Это запрет повторного закрытия при <=20,
  не нижний предел движения и не команда set_position20.
