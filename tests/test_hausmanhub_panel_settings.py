...
def panel_script(
    ...
    return f"""
      ...
      vm.runInThisContext(
        fs.readFileSync({str(LIGHT_PROTECTION_JS)!r}, "utf8").replace(/^import .*;\s*/gm, "").replace(/export /g, ""),
        {{ filename: {str(LIGHT_PROTECTION_JS)!r} }}
      );
      vm.runInThisContext(
      ...
      vm.runInThisContext(
        fs.readFileSync({str(ROOM_LIGHTING_JS)!r}, "utf8").replace(/^import .*;\s*/gm, "").replace(/export /g, ""),
