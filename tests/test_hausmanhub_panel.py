...
def test_room_lighting_configure_control_is_classified_and_styled_in_shadow() -> None:
    ...
    assert '@import url("./hausman-hub-room-lighting-editor.css?v=1.52.280")' in room_lighting_css
...
SCENARIO_DEVICE_PICKER_CSS = PANEL_JS.with_name("hausman-hub-scenario-device-picker.css")
...
MAX_DEVICE_INVENTORY_JS_BYTES = 24 * 1024
...
class PanelJavaScriptContractTest(unittest.TestCase):
    ...
    def test_tablet_library_pages_share_one_canonical_hero(self) -> None:
        ...
        self.assertNotIn(".hmh-library-hero-media", component)
        ...
        self.assertIn("hausman-hub-library-hero.css?v=1.52.280", panel_styles)
        for consumer in LIBRARY_HERO_CONSUMERS:
            source = consumer.read_text(encoding="utf-8")
            self.assertIn("createLibraryHero", source, consumer.name)

    def test_manual_room_badge_has_its_own_grid_row(self) -> None:
        styles = CLIMATE_OVERVIEW_CSS.read_text(encoding="utf-8")
    ...
    def test_panel_script_exists_and_stays_bounded(self) -> None:
        ...
        self.assertIn("hausman-hub-overview-utility-cards.js?v=1.52.280", overview)
        self.assertLessEqual(len(room_icons.encode("utf-8")), 12 * 1024)
        self.assertIn('hausman-hub-rollout.js?v=1.52.280', content)
        ...
        self.assertIn("hausman-hub-inventory-duplicates.js?v=1.52.280", device_inventory)
        self.assertLessEqual(
        ...
        self.assertIn('hausman-hub-energy-chart.js?v=1.52.280', energy)
        self.assertLessEqual(
        ...
        self.assertIn("hausman-hub-device-actions.js?v=1.52.280", content)
        ...
        self.assertIn("hausman-hub-scenario-catalog.js?v=1.52.280", scenarios)
        self.assertLessEqual(len(scenario_device_picker.encode("utf-8")), 16 * 1024)
        self.assertIn("hausman-hub-scenario-device-picker.js?v=1.52.280", scenarios)
        ...
        self.assertIn('hausman-hub-scenario-fields.js?v=1.52.280', scenarios)
        ...
        self.assertIn("contextualDeviceActionLabel", device_card)
        self.assertIn("conciseDeviceActionLabel", device_card)
        self.assertIn("DETAIL_ACTION_GENERIC_SEGMENTS", device_card)
        self.assertIn('return `${parts.join(" · ")}:', device_card)
        self.assertIn('"Открыть": "Открыть"', device_card)
        self.assertIn('inventory-device-state', device_card)
        self.assertIn("min-height:48px", device_card_css)
        self.assertIn(".physical-device-card {\n  min-height:150px;", device_card_css)
        ...
        self.assertIn('panel._connectionStatus || "loading"', overview)
    ...
    def test_device_detail_action_labels_remove_repeated_identity(self) -> None:
        script = r"""
          ...
          const target = { name: "Увлажнитель детская Humidifier" };
          const titles = [
            "Увлажнитель детская · Увлажнитель детская Humidifier · Включить",
    ...
    def test_overview_hero_uses_tablet_semantic_room_icons(self) -> None:
        ...
        self.assertIn(
            './hausman-hub-room-icons.js?v=1.52.280',
        ...
        self.assertIn("padding:2px 4px", overview_css)
        self.assertIn("scroll-padding-inline:4px", overview_css)
    ...
    def test_overview_home_target_and_grid_layout_contracts(self) -> None:
        ...
        self.assertIn('allowed.includes("set_home_targets")', climate_overview)
        self.assertIn('allowed.includes("synchronize_home")', climate_overview)
        self.assertIn("overview-canon-link is-tertiary", climate_overview)
        self.assertIn('climateActionRequest(panel, "set_home_targets", "hacs.climate.home", null,', climate_overview)
        self.assertIn("target_temperature: targetTemperature", climate_overview)
        self.assertIn("request.expected_control_revision = runtime.control_revision", climate_overview)
        self.assertIn('request.reliability_profile = "climate_reliability_v1"', climate_overview)
        self.assertIn("if (button.disabled || target === null) return;", climate_overview)
        self.assertIn("renderDashboardHeader(panel, connectionStatus, container, deps)", overview)
        ...
        self.assertIn("renderOverviewUtilityCards(panel, container, dashboard, deps)", overview)
        ...
        self.assertIn("--hmh-tablet-rail:238px", overview_css)
        self.assertIn("--hmh-tablet-rail-compact:88px", overview_css)
        self.assertIn("--hmh-tablet-side:var(--hmh-tablet-rail)", overview_css)
        self.assertIn("--hmh-tablet-rail-height:calc(100vh - 40px)", overview_css)
        self.assertIn("--hmh-tablet-hero:280px", overview_css)
        self.assertIn("--hmh-tablet-upcoming:minmax(180px,calc(", overview_css)
        self.assertIn(".overview-tablet-header h2 { margin:0; color:var(--hmh-text-primary); font-size:29px", overview_css)
        self.assertNotIn("vw", overview_css)
        self.assertIn("grid-template-columns:repeat(3,minmax(0,1fr))", overview_css)
        self.assertIn(".overview-tablet-sidebar", overview_css)
        self.assertIn("padding:2px 4px; overflow-x:auto; overflow-y:hidden", overview_css)
        self.assertIn("height:48px; min-height:48px", overview_css)
        ...
        self.assertIn("grid-row:1 / -1; grid-template-rows:var(--hmh-tablet-home-side) minmax(0,1fr)", overview_css)
        self.assertIn(":host(.overview-active) .overview-tablet-activity-detailed { display:flex; }", overview_css)
    ...
    def test_climate_bulk_restore_uses_the_existing_safe_action(self) -> None:
        ...
        self.assertIn('climateActionRequest(panel, "return_all_to_automatic", "hacs.climate.return-auto", null, {})', climate_overview)
        self.assertIn('action !== "return_all_to_automatic" && Number.isInteger(runtime.control_revision)', climate_overview)
    ...
    def test_generic_temperature_control_uses_runtime_step(self) -> None:
        source = (
    ...
    def test_panel_styles_are_local_and_stay_bounded(self) -> None:
        ...
        scenario_device_picker_styles = SCENARIO_DEVICE_PICKER_CSS.read_text(encoding="utf-8")
        ...
        self.assertLessEqual(len(scenario_device_picker_styles.encode("utf-8")), 8 * 1024)
        self.assertLessEqual(
        ...
        self.assertIn('hausman-hub-buttons.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-energy-chart.css?v=1.52.280', styles)
        self.assertLessEqual(len(rollout_styles.encode("utf-8")), 4 * 1024)
        self.assertIn('"/api/hausman_hub/panel/hausman-hub-panel.css?v=1.52.280"', content)
        ...
        self.assertIn('hausman-hub-settings.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-diagnostics.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-switch.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-notice.css?v=1.52.280', styles)
        ...
        self.assertIn('hausman-hub-device-maintenance.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-control-channel.css?v=1.52.280', styles)
        ...
        self.assertIn('hausman-hub-weather-sources.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-wizard-validation.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-catalog.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-media-device.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-device-card.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-device-controls.css?v=1.52.280', styles)
        ...
        self.assertIn('hausman-hub-scenarios.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-scenario-device-picker.css?v=1.52.280', scenario_styles)
        ...
        self.assertIn('hausman-hub-climate-overview.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-navigation.css?v=1.52.280', styles)
        self.assertIn('hausman-hub-kiosk.css?v=1.52.280', styles)
        self.assertIn(".kiosk-panorama-metrics", kiosk_styles)
        self.assertIn(".kiosk-panorama-intercom", kiosk_styles)
        self.assertIn('hausman-hub-rollout.css?v=1.52.280', styles)
        ...
        self.assertIn("--hmh-surface-canvas:#070B12", tokens)
        self.assertIn("--hmh-surface-canvas:#F3F6FA", tokens)
        ...
        self.assertIn("grid-template-columns:22px minmax(0,1fr) auto", devices_styles)
        self.assertIn(".devices-canon-workspace", devices_styles)
    ...
    def test_overview_upcoming_events_block(self) -> None:
        ...
        self.assertGreaterEqual(overview.count("renderUpcomingEvents(panel, container, deps)"), 2)
        ...
        self.assertIn(".overview-canon-upcoming-list::-webkit-scrollbar-thumb", styles)
...
THEME_TEST_HARNESS = """
  ...
  vm.runInThisContext(
    fs.readFileSync(__DEVICE_INVENTORY_JS__, "utf8").replace(/^import .*;\s*/gm, "").replace("export function renderDeviceInventory", "function renderDeviceInventory"),
  ...
  vm.runInThisContext(
    fs.readFileSync(__DEVICE_DISCOVERY_JS__, "utf8").replace(/^import .*;\s*/gm, "").replace(/export /g, ""),
  ...
  vm.runInThisContext(
    fs.readFileSync(__MEDIA_DEVICE_JS__, "utf8").replace(/^import .*;\s*/gm, "").replace(/export /g, ""),
...
class PanelThemeSwitcherTest(unittest.TestCase):
  ...
  const expectDayLight = (() => { const h = new Date().getHours(); return h >= 7 && h < 22; })();
  button.click();
  s = state();
        ...
        self.assertEqual(
            {
                ...
                "sidebar_title": "Hausman",
                "sidebar_icon": "mdi:home-heart",
                "module_url": "/api/hausman_hub/panel/hausman-hub-panel.js?v=1.52.280",
                "require_admin": True,
