...
- 2026-09-04: кандидат `1.52.211` объединяет актуальный `main` с защитой
  светового профиля после ручного выключения и contracts `0.65.0`. Устранены
  11 выпускных сбоев: обновлены клиентские снимки taxonomy/correlation,
  тестовый runtime мастера загружает модуль защиты, архитектурная проверка
  больше не принимает внутреннее имя receipt metadata за HTTP-библиотеку,
  а HACS fixture совпадает с каноническим контрактом. Полный локальный выпускной
  набор прошёл 2071 тест и 4 пропуска. Изолированный Chromium проверил 735 из
  735 элементов в 16 состояниях: 708 безопасно нажаты, 27 опасных действий
  намеренно заблокированы, ошибки и выход во внешнюю сеть отсутствуют. Android
  не меняется: новые API аддитивны и принадлежат панели HACS. Публикация и
  установка впереди.
