...
- 2026-09-04: кандидат `1.52.212` объединяет актуальный `main` с защитой
  светового профиля после ручного выключения и contracts `0.65.0`. Устранены
  11 выпускных сбоев. Проверка установки `1.52.211` дополнительно выявила
  конфликт собственного обработчика `OPTIONS` со штатным CORS Home Assistant:
  интеграция получила `setup_error`, поэтому production сразу возвращён на
  рабочую `1.52.210`. Исправление оставляет preflight штатному HTTP-серверу и
  закреплено отдельным регрессионным тестом. Повторный локальный выпускной
  набор прошёл 2071 тест и 4 пропуска. Chromium проверил 735 из 735 элементов:
  708 безопасно нажаты, 27 опасных действий намеренно заблокированы, ошибок и
  выхода во внешнюю сеть нет. Android не меняется: новые API аддитивны и
  принадлежат панели HACS. Публикация и установка `1.52.212` впереди.
