...
## Тамбур: быстрый возврат дневного света от присутствия и замка, 2026-09-16

- HACS `v1.52.293` (`7772d17`) разделяет реакцию на движение и присутствие:
  движение плавно ведёт люстру к 30% за 20 секунд, а отдельное присутствие,
  замеченное в этом окне, запускает быстрый набор до цели кривой. Без него
  начинается снижение к минимуму времени суток. Повторный пробный цикл требует
  реального motion-off, поэтому зависший `on` не разгоняет свет бесконечно.
- HACS `v1.52.293` опубликован и установлен. Живая config version 20 содержит
  оба нужных датчика. Целевые проверки: 160 passed, 50 subtests; после
  перезапуска `ha core check` PASS.
- HACS `v1.52.292` (`b324827`) разделяет минимальную яркость суточной кривой:
  15% до заката и 5% после заката. Снижение по отсутствию использует ту же
  границу. `v1.52.291` (`c9f38f0`) возвращает подтверждение присутствия к 15
  секундам, так как это минимальный интервал датчика.
- HACS `v1.52.292` опубликован и установлен. Целевые проверки: 157 passed,
  50 subtests; после перезапуска `ha core check` PASS.
- Контракт `v0.72.0` (`3daa0fb`) добавляет тип датчика `entry` только для
  сущностей `lock.*`. HACS `v1.52.290` (`6d41216`) уменьшает подтверждение
  присутствия суточной кривой с 15 до 8 секунд.
- Разблокирование `lock.aqara_smart_lock_a100` будит дневной профиль тамбура
  сразу, но не обходит ручную защиту и не отправляет команд замку. У A100 нет
  отдельного события фактического открытия полотна двери, поэтому триггер
  означает именно подтверждённое разблокирование.
- HACS `v1.52.290` опубликован и установлен. Конфигурация тамбура version 20
  содержит `entry_tambur_a100`; после перезапуска `ha core check` PASS.
  Полный local release gate: 3275 passed, 4 skipped, 1297 subtests.

## Тамбур: выпуск 1.52.289, 2026-09-16

- Контракт 0.71.0 (`v0.71.0`) добавил configurable возврат контура в авто,
  последовательности нажатий, яркость и оттенок. HACS 1.52.288 (`5686285`)
  реализовал их, 1.52.289 (`4c10117`) исключил `transition` из команд обычным
  выключателям. Именно этот параметр Home Assistant отвергал, прекращая
  команду целиком.
- Полный local release gate 1.52.289 PASS: 2363 unit, 3272 pytest, 4 skipped,
  1297 subtests, fixtures, Android compatibility, HACS package и safety.
  HACS v1.52.289 опубликован и установлен; core check после перезапуска PASS.
- В живом тамбуре config version 17: вторая клавиша зеркала возвращает комнату
  в авто, нижнее нажатие проходного выключает все источники, четыре верхних
  нажатия за 3 секунды добавляют люстру, точки, зеркало, затем люстру 100% и
  4000 K. Дневная цель 100%. После установки `transition`-ошибка не повторилась.
  Текущее live read-back источников unknown/off, поэтому физическое включение
  не имитировалось.

## HH-006/007: дневной переход тамбура и душевой, 2026-09-16

- По срочному поручению владельца подготовлен ремонт перед общей дорожной
  картой. База: установленная и опубликованная 1.52.286, commit 4de8dac.
  Рабочая ветка: codex/light-day-transition-2026-09-16.
- Дневной профиль больше не зависает целиком при неизвестном присутствии:
  новый дневной источник начинает с 5%, зеркало сохраняется до подтверждения
  основного света и параметров. Утреннее зеркало с 07:00, ночное до 00:00.
- Ручной свет не перехватывается по таймеру. Для выключенных ручных соседей
  обычного профиля устранена взаимная блокировка после выполнения условий
  защиты. Неизвестный свет не приравнивается к выключенному.
- Внутренние вызовы различаются по уникальному HA Context. Отказ датчика
  инвалидирует доказательство отсутствия. После ожидания питания повторно
  проверяются ручной приоритет и резерв другого исполнителя.
- Целевые проверки: 203 passed, семь файлов света, контракт 0.70.0 (8581cc9).
  Использовать HAUSMANHUB_CONTRACT_DIR с актуальными schemas/v1, старый путь
  по умолчанию не знает auxiliaries. Полный release gate прошёл: 2363 unit,
  3269 pytest, 4 штатных пропуска и 1297 subtests.
- Выпущен HACS 1.52.287: repair `19cb985`, release `292e31f`, tag
  `v1.52.287`. Версия установлена штатно без backup. Проверка Home Assistant
  до и после перезапуска прошла, update entity подтверждает installed/latest
  `v1.52.287`, JS/CSS панели отдают новую версию.
- Это VERIFIED, не приёмка живого дома: физические команды не отправлялись.
  Ручное владение, настройки, storage и физические устройства не менялись.
  Подробные результаты ревью и последующий commit: отчёт HH-006 в частном
  workspace, docs/product/roadmap/evidence/HH-006-2026-09-16.md. Следующий
  шаг: наблюдать разрешённый суточный переход и оба клиента без снятия защиты.

## Отсутствие по свежим датчикам во всех комнатах, 2026-09-15

- Правило «устаревший `on` игнорируется, отсутствие подтверждается свежими
  данными» распространено на общий движок комнат: `_presence_state` и
  `_absence_evidence`. Теперь и душевая, и другие комнаты со стандартным
  расписанием выключают свет по отсутствию, даже если один датчик «завис»;
  неизвестный/недоступный датчик по-прежнему блокирует (fail-closed).

## Тамбур: отсутствие по свежим датчикам, 2026-09-15

- Живой дефект: датчик присутствия тамбура перестал обновляться и «висел» в
  `on`, из-за чего отсутствие не подтверждалось и люстра не снижала яркость.
- Исправлено: устаревший `on` игнорируется (не присутствие и не отсутствие);
  присутствие подтверждается свежим `on`; отсутствие подтверждается, когда нет
  свежего `on`, нет unknown/unavailable и есть хотя бы один `off`. Классификация
  считается в runtime и передаётся в `evaluate_curve_room`, чтобы трекер и
  решение не расходились.

## Тамбур: датчики и подтверждение 15 с, 2026-09-15

- В конфиге тамбура (version 15) оставлены только «Датчик присутствия тамбур»
  (`binary_sensor.0xf044d3fffe0160c0_occupancy`) и вспомогательный «Датчик
  движения тамбур» (`binary_sensor.0xf044d3fffe11964d_occupancy`); датчик
  кабинета, ошибочно использовавшийся как присутствие тамбура, удалён.
  Датчик освещённости оставлен, он нужен блоку illumination.
- Подтверждение присутствия в суточной кривой: 15 секунд вместо 12
  (`LightCurveProfile.presence_confirm_seconds`, `CURVE_PRESENCE_CONFIRM_MS`).

## Тамбур: свежесть датчиков в суточной кривой, 2026-09-15

- Живой дефект: датчик присутствия тамбура провисел `on` ~65 минут без
  обновлений, а кривая учитывала `on` без свежести и возвращала люстру к
  максимуму, из-за чего пропадало снижение по отсутствию. Исправлено:
  присутствие учитывается только свежим (`<= 300` с), устаревший `on` —
  не присутствие; при неизвестном присутствии кривая удерживает текущую
  яркость (`hold`), а не выводит её на максимум.
- Напоминание: устаревшее состояние по правилам не считается ни присутствием,
  ни отсутствием, поэтому снижение по отсутствию требует свежих данных.

## Душевая: контракт 0.70.0, модель, передача и ревью, 2026-09-15

- Контракт 0.70.0: блок `auxiliary.exhaust` (порог 55, присутствие 120 с,
  отсутствие 300 с, выключение вытяжки 300 с), обязательность `fan` снята
  (`anyOf`); fixture душевой; счётчик 255→256. Валидатор PASS.
- Модель: `AuxiliaryExhaustPolicy`, `AuxiliaryPolicy.fan` стал optional,
  разбор/сериализация/ссылки/None-гарды; движок душевой `shower_exhaust.py`.
- Передача душевой подготовлена: `_shower_external`, исключение командных целей
  душевой из инвентаря координатора, гейт событий, recovery и общий гейт
  `_async_reconcile_room_due` для внешних сценариев (снят второй писатель от
  сработавшего таймера).
- Независимое ревью: код-ревью PASSED; проверка логики нашла три дефекта
  (вечный ручной блок кривой, потеря датчика как отсутствие, таймер переданной
  комнаты), повторная проверка — ещё два (неизвестный интервал попадал в
  отсутствие; тест гейта был ложноположительным). Все исправлены, тесты
  усилены; широкий профильный набор 331 passed, 3 subtests.
- Ручной блок кривой ограничен окном `minimum_interval_seconds` и не вечен;
  неизвестное присутствие не считается отсутствием и не наследуется.
- Осталось: наблюдение и dispatch вытяжки душевой в runtime (таймеры света и
  вытяжки раздельно), поля живого конфига душевой (датчик влажности, вытяжка),
  включение передачи, выпуск и живая проверка.

## Душевая: движок вытяжки и передача, 2026-09-15

- Реализован чистый движок вытяжки душевой `domain/shower_exhaust.py`:
  влажность > порога включает вытяжку сразу (даже при неизвестном присутствии);
  при присутствии и сухом воздухе взводится таймер присутствия (120 с), по
  истечении которого вытяжка включается; после ухода вытяжка выключается только
  через окно устойчивого отсутствия (300 с) и лишь при известной сухой
  влажности и подтверждённом владении. 13 тестов.
- Координатор обобщён на передачу душевой: `_scenario_external`,
  `_shower_external`, исключение целей душевой из `command_target_ids`,
  защита событийных путей и recovery. Пока не включено в `__init__.py` —
  сначала нужно подключить вытяжку в runtime и конфиг.
- Свет душевой уже воспроизведён в конфиге runtime (день/вечер/ночь, отсутствие
  300 с), но команды всё ещё резервирует старый координатор.
- Осталось: наблюдение влажности/вытяжки и dispatch в runtime, поля конфига
  (датчик влажности, вытяжка, политика), включение передачи, выпуск и живая
  проверка.

## Суточный алгоритм тамбура подключён, 2026-09-15

- `profile: "day_curve"` (контракт 0.69.0) включает встроенный алгоритм:
  движок `domain/light_curve.py` + адаптер `evaluate_curve_room` в
  `room_lighting_engine.py`. Runtime ведёт таймеры присутствия (12 с
  подтверждения) и отсутствия, команды идут через обычный dispatch с плавными
  переходами (`transition`). Споты (`autoControl=false`) не затрагиваются.
- Правка: явное расписание `off` и `day_curve` не блокируются отсутствием
  записей владения; подтверждённое ручное действие по-прежнему приоритетно.

## Алгоритм освещения тамбура по файлу владельца, 2026-09-15

- Разобран `Алгоритм_освещения_тамбура.xlsx` (лист «Алгоритм освещения» +
  «Пояснения») и реализован чистый движок `domain/light_curve.py`.
- Алгоритм: режим суток задают время, рассвет и закат; присутствие никогда не
  переключает режим и не меняет цвет. Ночь 23:00–00:30 — люстра выключена,
  зеркало включено; 00:30–max(рассвет, 09:00) — всё выключено; 09:00 — 5%;
  09:00–12:00 — плавный рост до 100% и нейтральный свет; 12:00–закат−60мин —
  100% нейтральный; закат−60мин–22:00 — спад до 5% и потепление;
  22:00–23:00 — 5% тёплый. Отсутствие 5 минут запускает 5-минутный спад к 5%;
  подтверждённое 12 секунд присутствие возвращает яркость к максимуму текущего
  режима за 10 секунд от фактической яркости.
- Параметры вынесены в `LightCurveProfile` (нейтральный 3000 K и тёплый
  2200 K — как в живой конфигурации тамбура). 13 тестов покрывают каждую
  строку таблицы.
- Не подключено к runtime: для работы нужно отметить комнату профилем в
  конфигурации (совместимое поле) и генерировать команды люстре/зеркалу из
  решений кривой с учётом таймеров присутствия/отсутствия. Это следующий шаг.

## Миграция света: движок ванной и fail-closed тамбур, 2026-09-14 (незакоммичено)

- Первый инкремент переноса комнат: чистый auxiliary engine ванной
  `domain/room_lighting_auxiliary.py` без импортов Home Assistant. Воспроизводит
  legacy-контроллер: три полосы времени, включение по свету/влажности,
  выключение только при подтверждённом владении, таймер `bathroom_day_off`,
  отбраковка нечисловой (NaN/inf) влажности как недостоверной.
- Паритет с legacy-координатором покрыт тестами: день (включение, таймер,
  выключение по таймеру), ночь (включение, выключение, неизвестная влажность),
  тихий режим (первая лампа, вторая лампа, неизвестный свет).
- Ремонт передачи тамбура дополнен fail-closed защитой: координатор отвечает
  `ready=False` (`externally_managed`) на любой запуск сценария тамбура,
  включая ручной, — устаревшее действие замороженной записи больше не может
  быть исполнено flow. Generic-адаптеры событий и расписаний получают
  `owned_scenario_ids | externally_managed_scenario_ids`, поэтому переданный
  сценарий остаётся у них в исключениях (иначе `manual_chandelier_on` запускал
  бы flow вторым исполнителем против `RoomLightingRuntime`).
- Проверки: профильный набор (движок, комнаты, адаптеры, startup, runtime,
  engine, ownership, domain, service, shadow) — 241 passed, 3 subtests;
  `py_compile` и `git diff --check` — чисто. Регрессионные тесты: NaN/inf
  отклоняются; контекст тамбура fail-closed, ванная остаётся `ready=True`;
  переданные id публикуются для исключений адаптеров.
- Контракт 0.68.0 (ветка `codex/bathroom-auxiliary-contract-2026-09-14`,
  рабочая копия `worktrees/codex-bathroom-auxiliary-contract-2026-09-14`,
  незакоммичено): совместимая модель вспомогательного контура — optional
  `devices.auxiliaries` (`kind: "fan"`), sensor `kind: "humidity"`,
  optional `auxiliary.fan` с `humidityThreshold`/`dayOffSeconds` и границами
  полос. Валидатор: 103 схемы, 255 fixtures PASS; fixture
  `room-lighting-config-bathroom-auxiliary.json` добавлен.
- Доменная модель интеграции разобирает новые поля: `AuxiliaryTarget`,
  `AuxiliaryFanPolicy` (границы полос с проверкой порядка), `AuxiliaryPolicy`,
  разбор/serialization в `room_lighting.py`, humidity-датчик на `sensor.*`.
  Вентилятор учитывается как control entity в коллизиях комнат.
  `bathroom_auxiliary_inputs()` в движке даёт чистые входы (порог, таймер,
  полосы) для будущего теневого подключения. Fixture контракта разбирается
  доменной моделью, профильный набор с новой схемой: 289 passed, 3 subtests.
- По итогам ревью: `apply_template` с `keepDevices` больше не теряет
  auxiliary-политику комнаты — устройства и их политика переносятся вместе
  (регрессионный тест в `test_room_lighting_storage.py`). Схема и серверная
  проверка выровнены: humidity-датчик в схеме допускает только `sensor.*`
  (`if/then`), требование порядка полос явно объявлено серверным.
- Доказательная parity ванной: исчерпывающий харнесс
  `tests/test_room_lighting_auxiliary_parity.py` перебирает всё пространство
  решений (3 полосы × 3 состояния двух ламп × 3 значения влажности × 3
  состояния вытяжки × владение) — 324 случая, все ветки (включение,
  выключение, hold, idle, unknown, отложенный таймер) совпадают с legacy.
- Теневая запись: runtime считает и журналирует auxiliary-решение комнат с
  политикой, не отправляя команд (`build_auxiliary_observation` в
  `room_lighting_ha_state.py`, `async_record_auxiliary` в
  `room_lighting_shadow.py`, хук в `_process_room`). Комната без политики или
  без ровно двух световых целей пропускается fail-closed; владение берётся из
  журнала runtime (пока writer — legacy, оно false, что записывается честно).
  10:00 UTC, режим `day`, свет горит, влажность 70 → движок просит вытяжку,
  команд нет — проверено тестом.
- Контракт 0.68.1 и модель: политика вытяжки получила optional упорядоченный
  `lightTargets` (ровно две лампы). Наблюдение выбирает явные лампы, а без них
  привязывает вытяжку к целям комнаты только при ровно двух. Это снимает
  блокер для живой `dop_vanna` с тремя световыми целями.
- Живой дефект тамбура (2026-09-15): после передачи комнаты и перезапуска
  включённая лампа без записей владения считалась ручной, поэтому зеркало не
  выключалось утренним расписанием. Исправлено: явная запись расписания с
  режимом `off` выполняется и без подтверждённого владения, подтверждённое
  ручное действие по-прежнему блокирует; выключение по отсутствию и
  автоматическое включение остаются строгими.
- Не выполнено: смена единственного writer ванной (журнал команд и таймеры
  auxiliary в dispatch-контуре), обновление `docs/COMPATIBILITY.md` (пара
  интеграция ↔ контракты 0.68.0) при выпуске, Android-толерантность к новым
  полям. Коммиты, публикация и установка — по отдельному решению владельца.

## Выпуск 1.52.277: единственный writer света, 2026-09-14

- `b482741` опубликован как HACS `v1.52.277` и штатно установлен. Полный
  выпускной набор: 3119 passed, 4 skipped, 1297 subtests; финальное ревью и
  security audit: PASS.
- `RoomLightingRuntime` fail-closed не посылает команду при пересечении цели,
  entity или питания с `ScenarioControlCoordinator`. В живой проверке люстра
  тамбура осталась выключенной 30 секунд при активном присутствии.
- Небезопасный глобальный redeploy Node-RED душевой не выполняется. Устаревший
  managed flow душевой требует отдельного contract-first isolated redeploy.

## Шаг 5b-fix освещения комнат: правки редактора, 2026-09-11

- Применение шаблона больше не ломает сохранение: после ответа шаблона
  редактор перечитывает `GET /config` и переносит актуальные `version`/
  `updatedAt` хранимого документа в черновик; сценарий «404 → создать из
  шаблона» сохранён.
- Добавлен переключатель комнатного `autoAdopt` рядом с per-device overrides.
- Пресеты устройств/записи расписания больше не создают `entityId: ""` и
  выбирают следующий свободный id без дублей.
- Проверка: `node --check` по изменённому JS — OK.

## Шаг 5b освещения комнат: HACS-редактор, 2026-09-11

- Добавлен `frontend/hausman-hub-room-lighting.js` + `.css`: блок
  «Автоматическое освещение» в карточке комнаты. Блоки: шаблоны, устройства
  (датчики/цели/питание/беспроводные, `autoAdopt` и override), расписание
  (`when`/`targets`/`how` с `minOnSeconds`), привязки выключателей, люкс,
  плавное гашение, ручная защита, уход/возврат, live-тест 30 с с шагами.
- Редактор общается с API Шага 5a через `panel._hass.callApi`
  (`GET/PUT .../lighting/config`, `GET .../templates`, `POST .../templates/apply`,
  `POST/GET .../live-tests`); истина — ответ сервера, ошибки показываются
  русским сообщением.
- Модуль подключён импортом в `hausman-hub-rooms.js` и вызовом в
  `openRoomOverview`; CSS добавлен `@import` в `hausman-hub-panel.css`.
  Существующие модули не менялись по логике.
- Проверка: `node --check` для `.js` (через `.mjs`) — OK. Браузерных тестов
  в окружении нет.
...
- `domain/room_lighting_engine.py`: чистая функция `evaluate_room_lighting`
  ...
  гашение; фактический приоритет: ручное > защита > расписание > люкс >
  присутствие > гашение.
- Жёсткие окна тамбура убраны: режим активной записи расписания задаёт
  поведение цели (`off`/`always`/`night_light`/`on_presence`), а
  `how.minOnSeconds` — минимум горения для `night_light`. Движок ничего не
  дорисовывает за конфигурацию.
...
- Проверки: `tests/test_room_lighting_engine.py`,
  ...
  `tests/test_room_lighting_domain.py`, `tests/test_room_lighting_storage.py`
  — 60 PASS; полный набор не запускался.
...
- В климатической панели добавлена кнопка «Вернуть устройства в климат.
  контур». Она показывается только при существующих ручных исключениях и
  только когда runtime явно разрешает `return_all_to_automatic`.
- Кнопка использует уже проверенный запрос v1: состояние передаётся с
  correlation ID и revision, повторное нажатие на время операции отключено,
  а результат перечитывается с сервера. Цели температуры и влажности, а
  также физические команды устройствам она не меняет.
- Полный допуск: 2884 PASS, 4 штатно пропущены, 1293 subtests PASS; local
  release: 2327 unittest, 4 штатно пропущены и 72 pytest PASS; critical
  coverage 75%; browser gate 50 PASS без непредусмотренных сетевых запросов
  или изменений живого дома. GitHub Actions `34370040394` PASS.
- Публичный выпуск `v1.52.238` создан из `184518c` без assets и установлен
  точным `update.install`. После одного restart Home Assistant запущен,
  панель отдаёт `1.52.238`, climate runtime свежий, активных операций нет.
  Новых климатических команд не было. Подробности:
  `docs/RELEASE_AUDIT_1.52.238.md`.

## Выпущен и установлен возврат комнат в автоматический контур 1.52.237, 2026-09-09

- Исправлена одна граница ручного климатического владения: возврат конкретной
  комнаты в automatic теперь снимает и ручные отметки всех её настроенных
  приборов. Режим комнаты и разрешение на отдельную цель больше не расходятся.
  Другие комнаты и их исключения не меняются.
- TDD-регрессия подтверждает сохранение режима, очистку ручных отметок и
  пустой список физических вызовов. После выпуска live-действия
  `set_room_mode=automatic` для комнат Игоря и Алисы завершились: control
  revision 26 -> 28, активных операций нет. У обеих комнат снова доступна
  отдельная температура; у комнаты Игоря также доступна влажность согласно
  её текущему составу приборов.
- Полный pytest: 2883 PASS, 4 skipped, 1293 subtests PASS. Local release:
  2326 unittest, 4 skipped и 72 pytest PASS. Критическое покрытие 75%.
  Browser gate: 50 PASS, 43 исходные точки, 735 сигнатур, 708 безопасно
  выполненных действий и 27 намеренно заблокированных, без внешней сети или
  непредусмотренных изменений.
- GitHub Actions `34352266387` PASS. Выпуск `v1.52.237` опубликован без
  assets, точная HACS-установка подтверждает installed/latest `v1.52.237`.
  Проверка конфигурации и один restart прошли. После короткого прогрева
  read-only smoke PASS: climate fresh, commands enabled, active operations 0,
  максимальный ответ 303 мс. Подробности: `docs/RELEASE_AUDIT_1.52.237.md`.

## Живая проверка целей после 1.52.236, 2026-09-09

- На Lenovo с Android `1.0.278` через обычный экран изменена общая цель с
  25,0 на 25,5 °C. Операция завершилась, и все пять управляемых комнат
  сохраняют 25,5 °C. Затем HACS принял отдельные операции для спальни,
  кабинета и кухни; каждая завершилась без активного запроса.
- Отдельная температура для комнаты Игоря и комнаты Алисы недоступна по
  текущей серверной области управления. Попытка в UI не изменила состояние,
  а обход защиты не выполнялся. Это не ошибка выпуска.
- Свежий read-only smoke PASS: climate runtime отвечает 200, снимок свежий,
  командный контур доступен, активных операций нет. Это доказывает
  сохранение цели и завершение квитанций, но не заявляет физическое
  подтверждение каждым прибором.
- HACS `1.52.236` и Android `1.0.278` установлены. Сервер уже выдаёт
  `return_all_to_automatic`, но в Android-клиенте нет элемента для этого
  действия. Нужна отдельная клиентская доработка и выпуск Android.

## Выпущен и установлен repair climate runtime 1.52.236, 2026-09-09

- `ClimateTabletService` безопасно исправляет только authenticated-запись
  общей цели с корректными квитанцией и checkpoint: ошибочный
  `accepted_timeout` переводится в `terminal_mixed`. Запрос, квитанция,
  счётчики, ручное владение, область действия и запрет повтора не меняются;
  исполнитель климатических приборов не вызывается.
- Pin Contracts приведён к уже встроенному каноническому `0.65.2`
  (`69fe733`). Три schema-hash проверки исключают повторное расхождение
  версии и содержимого.
- До публикации прошли 420 профильных тестов, 400 subtests, полный pytest
  (2882 PASS, 4 skipped, 1293 subtests), local release (2325 unittest и
  72 pytest), critical coverage 75% и 50 browser tests. GitHub Actions
  `34341637755` PASS на `cd59b79`.
- Выпуск `v1.52.236` опубликован без assets и установлен точно. Проверка
  конфигурации прошла, Home Assistant восстановился после одного restart.
  Read-only smoke PASS: climate runtime 200, снимок свежий 53 мс, активных
  операций нет, физические команды не отправлялись.
- Первый отменённый запрос резервной копии кратко передал учётные данные в
  локально наблюдаемую командную строку. Значение не записывалось в исходники
  или документацию; последующие запросы использовали защищённую конфигурацию.
  Нужна отдельная ротация административного токена Home Assistant.

## Диагностика climate runtime: один некорректный служебный статус записи, 2026-09-09

- После установки `1.52.235` интеграция, конфигурация, пять комнат и
  нативный климатический runtime доступны. Планшетный климатический сервис
  отключён fail-closed: `/climate/runtime` возвращает 503 `unavailable`.
- Read-only проверка защищённой истории нашла ровно одну последнюю запись
  общей цели. Её квитанция, область действия, подпись, checkpoint и состав
  из пяти комнат корректны. В ней 2 подтверждённые ветви, 1 отложенная,
  4 ручные и 1 ветвь с истекшим подтверждением.
- Эта смешанная запись сохранила `dispatch_ledger.state=accepted_timeout`.
  Такой статус допустим лишь для тайм-аута и подтверждённых ветвей. Для
  фактического смешанного результата нужен `terminal_mixed`; строгая загрузка
  поэтому отклоняет историю и не создаёт планшетный сервис. Это локальный
  дефект классификации сохранённого результата, не сбой Home Assistant,
  устройств, registry, ключа или новой версии `1.52.235`.
- Никаких повторных установок, restart, изменений хранилища или физических
  климатических команд при диагностике не было. Следующий безопасный шаг -
  подготовить узкий repair с воспроизводящим тестом и миграцией только этой
  полностью проверенной записи, не переотправляя команду устройствам.
  Подробности: `docs/migration/CLIMATE_RUNTIME_LEDGER_DIAGNOSIS_2026-09-09.md`.
...
- Добавлено additive-действие v1 `return_all_to_automatic`. Оно доступно
  только когда серверное домашнее управление разрешено и в текущем контуре
  есть ручное исключение. Запрос не принимает параметры или enhanced
  reliability-профиль.
- Одна атомарная серверная операция снимает ручное владение только у комнат
  и приборов текущего контура, сохраняет память один раз, не посылает
  физических команд и не меняет цели температуры или влажности. Наблюдения
  direct Wi-Fi, атрибуция и ручные исключения вне контура сохраняются.
- Исходники подготовлены, но не опубликованы, не установлены и не применялись
  к живому дому. Для фактического выпуска нужна отдельная команда владельца.
- Проверено локально: 2324 unittest PASS, 4 skipped, 72 pytest PASS,
  local release PASS, critical coverage 75% при пороге 75%, 50 браузерных
  сценариев PASS. Контракт 0.65.2 и private Android 1.0.278 образуют
  подготовленную исходную комбинацию.

## Канонический ответ отложенных приборов, 1.52.233, 2026-09-09

- Живая совместная цель на 1.52.232 сохранена с планшета: 25.5 °C и 45%
  во всех пяти управляемых комнатах, revision 18. Два прибора confirmed 1/1,
  четыре ручных и два отложенных 0/0. Полный физический успех не заявляется.
- Строгая проверка выявила прежний текст deferred, не совпадающий с API v1.
  Новые ответы используют канонический текст. Старые проверенные квитанции
  исправляются только в HTTP-копии, без записи в подписанную историю.
  Статусы, счётчики, доказательства и нулевые вызовы сохраняются.
- Проверки сначала воспроизвели дефект, затем прошли. Полный pytest:
  2878 PASS, 4 skipped. Полный local release: 2321 unittest PASS и остальные
  проверки PASS. Критическое покрытие 75% при пороге 75%.
- Комплект VERIFIED_LOCAL: 981 файл и права совпадают с исходниками.
  Браузер: 50 PASS, 735 состояний элементов, 708 действий, 43 исходные точки,
  ошибок и непредусмотренных запросов нет. Contracts 0.65.0 и private Android
  1.0.277 совместимы и не менялись. Frontend меняет только номер кеша.
- Самостоятельное ревью Codex: узкая нормализация только точного старого
  текста при строгих нулевых счётчиках, без обхода проверки истории и доступа.
  Неизвестный текст и повреждённые поля не исправляются автоматически.
- 1.52.233 опубликован из 097c13c после GitHub 34308045860 PASS и установлен.
  Все 600 файлов и панель совпали, конфигурация и один restart PASS.
  Обе цели пережили перезапуск, квитанция проходит строгую схему,
  подписанная запись побайтно прежняя. Повторных команд нет. Полный backup
  5a41aef4 и свежие защищённые архивы на сервере сохранены.
- Расширенный аудит полного runtime нашёл прежнее расхождение с его схемой:
  дополнительные desired-поля и отсутствующие старые target-поля приборов.
  Эти участки существуют с 4c2dce02 / 1.52.196 от 31 августа и не менялись.
  Приёмка сохранения цели PASS, полный live runtime schema audit FAIL.
  Это отдельная задача согласования контракта, подробности в корневом
  docs/migration/CLIMATE_RUNTIME_CONTRACT_DRIFT_2026-09-09.md.

## Совместная цель для смешанного состава комнат, 1.52.232, 2026-09-09

- Температура и влажность выбирают реальные группы приборов независимо
  по комнатам. Отсутствие увлажнителя не блокирует температурную цель.
  Повреждённые привязки и полное отсутствие запрошенной группы отклоняются.
- Ручные приборы сохраняются в проверке результата с нулём физических
  вызовов. Состав общей команды берётся из проверенных исполнителей,
  а не из промежуточного расчёта автоматики, который может пропустить комнату.
- Устаревшая влажность не мешает температурному прибору только при отдельном
  доказательстве свежей температуры. Общая оценка комнаты не переписывается,
  старые нужные показания и неизвестное доказательство не допускаются.
- Десять новых регрессий PASS: пять смешанных комнат, ручные исключения,
  точные 25.5 °C и 45%, сохранение после restart, отсутствие повторов,
  старые показания и независимость состава от промежуточного расчёта.
- Полный pytest 2876 PASS, 4 skipped; полный local release PASS;
  критическое покрытие 75% при пороге 75%; браузер 50 PASS и точный отчёт
  735 состояний элементов / 708 действий / 43 исходные точки PASS.
  Локальный комплект VERIFIED_LOCAL, 981 файл и права совпадают с исходниками.
- Самостоятельное ревью Codex: проверены границы параметров и владения,
  строгие отказы, сохранение после перезапуска, запрет повтора и API v1.
  Frontend содержит только замену версии кеша. Contracts 0.65.0 и
  private Android 1.0.277 остаются совместимыми, их код не менялся.
- На сервере пока 1.52.231. Полный backup 5a41aef4 за текущий день HA
  повторно проверен; точные архивы компонента и девяти климатических файлов
  сохранены на сервере. Публикацию, установку и живую приёмку фиксирует корень.

## Подтверждение режима при обычном ходе часов 1.52.231, 2026-09-08

- Живая проверка 1.52.230 выявила ложный pending_expired после успешного
  сохранения режима. Между записью и чтением обновлялось время общей памяти,
  хотя выбранный режим не менялся. Прежние тесты фиксировали часы.
- Подтверждение теперь допускает более новое долговечное чтение того же
  режима. Чтение до записи и несовпадающий режим по-прежнему отклоняются.
  Область действия, отдельные подписи, нулевые физические счётчики и запрет
  повторного исполнения не менялись. API v1 и Android 1.0.277 совместимы.
- Регрессия сначала воспроизведена, затем исправлена. Проверки движущихся
  часов охватывают все четыре исполнительных типа, комнату, фоновый запрос
  и восстановление после перезапуска. Девять профильных случаев PASS.
- Полный pytest: 2866 PASS, 4 skipped; полный local release PASS;
  критическое покрытие 75% при пороге 75%. Все 50 браузерных тестов PASS:
  735 действий, 43 исходные точки управления, ошибок нет. Точная проверка
  сетевого отчёта PASS.
  Точный локальный комплект VERIFIED_LOCAL, все 980 исходных файлов
  побайтно совпадают с ним; смены прав нет.
- На сервере установлен 1.52.230. Все 600 файлов совпали с выпуском,
  загрузка подлинной истории и live smoke PASS. Перед следующим обновлением
  сохранены точный архив компонента и защищённая климатическая история.
  Публикацию и живую проверку 1.52.231 фиксирует корневой контекст.

## Исправление подтверждения режима и истории 1.52.230, 2026-09-08

- Сохранённый ручной/автоматический режим подтверждается по долговечной
  серверной записи с точными нулевыми счётчиками физических команд. Все
  четыре исполнительных типа поддержаны; отдельное HA-событие не требуется.
- Повторное чтение не переписывает доказательство уже подтверждённого
  устройства. Новый native-флаг подтверждения принимается только при
  совпадении цели и времени наблюдения с точным запросом.
- Загрузчик ограниченно исправляет противоречие старого writer только после
  проверки полного состояния и отдельного checkpoint операции/области.
  Он снимает недоказанный успех, но сохраняет цель, принятие команды и
  запрет повторной отправки. Подделка и неполная подпись отклоняются.
- Проверено: 2864 pytest PASS, 4 skipped; local release PASS;
  critical coverage 75% при пороге 75%; семь новых regression cases PASS.
  Все 13 native-записей текущего сервера проходят загрузчик без изменений.
...
- Самостоятельное итоговое ревью Codex: проверены точные границы доверия,
  сохранение доказательств, zero-call режимы, запрет повтора и API v1.
  Поведение frontend не менялось: сверена точная замена только версии кеша.
- Совместимый клиент: private Android 1.0.277 build 281, contracts 0.65.0.
  На сервере пока установлен 1.52.229; перед вторым обновлением сохранены
  архив компонента и защищённая копия климатической истории на самом сервере.
  Публикацию и установку нового выпуска фиксирует корневой контекст.

## Исправление установки климатической цели 1.52.229, 2026-09-08

- Надёжный climate/actions подтверждает сохранение цели до ответов приборов.
  Выполнение принадлежит серверу и не отменяется при разрыве HTTP; дубликат
  запроса не посылает команды повторно. Сохранённая квитанция и временная
  проекция доступны во время медленной отправки. При unload задача завершается
  с сохранением границ уже начатого исполнения.
- Ручное включение всех четырёх видов климатических приборов возвращает
  автоматический режим, выключение задаёт ручной. Атрибуция собственных
  команд сохраняется через все изменения памяти и перезапуск.
- Общая цель берётся только из комнат контура. Датчики и чужие оси управления
  не создают ложное ожидание применения. Частичные физические результаты
  записываются до возврата, без потери подтверждений исправных устройств.
- Проверено на закреплённом окружении Python3.13: 2857 passed,4 skipped;
  local release PASS; critical coverage75%; browser50 PASS; полный runtime
  отчёт43 source sites PASS. Локальный комплект VERIFIED_LOCAL, без дрейфа.
- Самостоятельное итоговое ревью Codex: проверены отмена HTTP, долговечность
  квитанции, повторные запросы, границы сохранения и атрибуции, совместимость
  API v1. Контракты и физические защиты не ослаблены.
- Клиентская часть подготовлена отдельно в private Android1.0.277:
  защищённый домофон и отдельный статус сохранения цели. Публикация и
  установка разрешены владельцем; их фактический результат фиксирует корень.

## Конечный кандидат 1.52.227, 2026-09-08

- Независимый допуск940e1f2 BLOCKED: два полных pytest дали1 failed,
  2838 passed,4 skipped,1238 subtests. Одинаковый TimeoutError на
  test_restart_mode_restarts_the_five_minute_timer line2116, ожидание0.1s.
  Local release и critical PASS; browser46 passed/1 interrupted, не PASS.
  Ограниченная диагностика завершена: точный тест отдельно PASS, но
  причина0.158s/0.148s callback неизвестна. Runner3.12.3/9.1.1/1.4.0
  отличается от CI3.13/8.3.5/1.3.0. Нужен новый допуск в закреплённом
  окружении; исходники и timeout по этому сбою не менялись. Все агенты
  завершены, новых review/security и публикации/установки нет.
- P1 настоящего перезапуска исправлен в50c075b. Завершённая native migration
  сверяет стабильные state/automationId/definitionHash всех18 правил;
  точный context/time CAS для prepared/reconcile/rollback не ослаблен.
  Два RED-случая воспроизведены до правки, после неё native и полный
  coordinator restart проходят без повторной записи, вызовов и изменения
  Node-RED; все восемь контроллеров запускаются один раз.
- Worker завершён, runtime diff ограничен native_automation_migration.py
  и двумя тестовыми файлами. Полный pytest2839 passed/4 skipped за342.03s,
  migration profile115, local release PASS, critical75.25% при пороге75,
  postcommit targeted21 и package PASS. Root сверил исходные итоги,
  самостоятельный diff и повторил package PASS на чистом50c075b.
- Восемь activation_ready в manifest разрешены явно с7a406ae;
  безопасный default=False сохранён. Версия1.52.227 ещё не опубликована,
  рабочая система1.52.226 не обновлена. Старый кандидат7a406ae имел
  пройденные тесты, но не получил допуска из-за P1; его нельзя публиковать.
- Новый допуск нужен до reviewer и security_auditor. После исправления
  собран комплект1132 файлов с hash3e33d4e19b236de1e801459ed9cced8157aa5d604a46bd8573634705f028578a,
  VERIFIED_LOCAL и sourceDrift[], но это не допуск к публикации.
- Полное ревью05a323a..7a406ae уже выполнено: локальное Codex PASS,
  security выявил только исправленный P1. Внешний helper завершился
  exit124 по тайм-ауту600s без заключения, Review: fallback.
  Для повторного ревью основная дельта7a406ae..50c075b: три файла,
  отдельный completed helper и регрессии восстановления. Пустой рабочий
  git diff не является областью ревью; проверять коммитную дельту.
- План исправления в корне проекта:
  docs/migration/NATIVE_RESTART_EVIDENCE_REPAIR_2026-09-08.md.
  Вода и автоматы исключены также косвенно. Испытания света только
  кабинет/малый коридор/душевая при необходимости, голос отменён.
  Шкала кухни не подтверждена, вытяжка кладовки не привязана.

## История предыдущих инкрементов

- Климатический инкремент завершён в d251423. Полный набор после последней
  правки: 2830 passed, 4 skipped за 340.35s; root прочитал исходный вывод
  и независимо повторил package PASS на чистом коммите. HTTP-матрица:
  137 passed. Безопасные climate/humidifier команды имеют отдельного
  сохраняемого владельца, ограниченное ожидание, защиту reload и replay.
  SmartIR не получает физическое подтверждение по программному эхо.
  Климатические цели и правило 26°C не менялись. Версия остаётся 1.52.226,
  все activation_ready=False. Следующий ограниченный этап: конечные
  версия и активация, затем tester, reviewer/security и разрешённый выпуск.
  Рабочая система не обновлена, физических команд в этом инкременте нет.

- Точная матрица миграции завершена в ae641b6: исходные 58/46,
  подготовленные 63/17, завершённые 63/22; отключаются точные 29,
  полностью сохраняются 24, обновляются две ручные сцены штор.
  Root сверил полный вывод 2805 passed, 4 skipped, 1238 subtests
  за 306.48s и повторил package PASS на чистом коммите. Baseline
  сравнивает записи независимо от порядка, CAS сохраняет проверку порядка.
  На свежем чтении live baseline совпал, исходный реестр не изменён.
  Далее отдельный климатический инкремент по корневому плану
  docs/migration/CLIMATE_LATENCY_IMPLEMENTATION_PLAN_2026-09-07.md.
  Все activation_ready=False, версия 1.52.226, установки нет.
  Конечные версия и активация нужны до tester/review/security.
...
- Сохраняемый допуск кабинета завершён в8d978a4. Root сверил исходный
  ...
  MQTT/Z2M, штатная форма confirm/revoke, проверяемое хранилище и
  динамическая политика подключены; разрешение кухни не добавлено.
  Тот же единственный writer продолжает отдельный optimistic-evidence
  инкремент, затем общую приёмку7. Климат и конечные activation/version
  ещё впереди. Tester/review/security нового прохода не начаты.
  Production1.52.226 не обновлён. Root по новому прямому запросу владельца
  поставил live manual-off hold тамбура до17:42:29.632489Z,1800s:
  люстра и реле1/2 OFF, зеркалоL1 ON. Не снимать этот запрет при установке.

- 2026-09-07:6.3/6.4 в9e17d50, исходный полный результат проверен root:
  2760 passed,4 skipped,1238 subtests за342.94s; профиль125 PASS.
  Root повторил package PASS. Владелец физически подтвердил кабинет0,
  примерно50 и90 и разрешил текущую шкалу без новой калибровки.
  Локальный следующий шаг: отдельное сохраняемое подтверждение exact
  office identity через admin options, затем общая приёмка7. Кухня пока
  не подтверждена. План2026-09-07-curtain-scale-confirmation.md.
  Установки/активации нет; motor limits/reverse не меняются.

- 2026-09-07, части6.1/6.2 завершены в `77592fa`: общий путь команд
  четырёх штор, отдельные requested/applied/observed и честные квитанции.
  После последней правки: 2738 passed, 4 skipped, 1238 subtests за303.27s;
  профиль64 PASS. Root сверил исходный вывод и повторил package PASS
  на чистом worktree. HTTP-тест проходит настоящий service/executor
  с fake HA:100->80, replay после cap70 не двигает мотор повторно.
  Недатированная позиция не разрешает no-op или закрытие кабинета.
  Следом части6.3/6.4: сохраняемая личная защита, четыре цели и сцены.
  Все activation_ready=False, шкала кухни/кабинета не подтверждена,
  production1.52.226 не менялся. Физических команд не было.

- 2026-09-07, часть 5 завершена в `9b127c3`: душевая, туалет, ванная,
  кабинет, 26 файлов. Полный pytest: 2679 passed, 4 skipped,
  1238 subtests за 302.33s. Package PASS до и после commit. Исправлены
  partial-restart source replacement душевой и повтор неподтверждённого
  выключения ванной после новых показаний/restart. Все источники
  runtime/tools совпадают. Browser этой части отдельно не повторялся,
  его полный повтор остаётся общей приёмке. Activation_ready закрыт.
  Root запросил read-only уточнение части 6 по совместимой квитанции
  ограниченной позиции штор; worker остановлен после handoff.
  Нет установки и физических действий. Production остаётся 1.52.226.

- 2026-09-07, части ручной защиты/питания и двух коридоров сохранены
  в `4ea978b`. На точном commit: 2640 passed, 4 skipped, 1238 subtests,
  309.79s; browser 47 passed, 16 состояний/735 элементов без ошибок.
  Package PASS после commit, восемь runtime hashes совпадают с manifest.
  Реализованы сохраняемые переходы яркости, manual/lux/absence state,
  ночные границы, точное серверное действие, проверка поколения перед
  реле и лампой. Питание подтверждается last_reported/last_changed,
  не изменением постороннего атрибута. Исправлен restart/CAS staging:
  точные hashes/key sets/topology, проверяемый откат обоих источников.
  Это локальный checkpoint исполнителя. Release version check требует
  повышения 1.52.226 перед будущим выпуском; версия сейчас не повышена.
  Все activation_ready=False. Далее часть 5 (остальные комнаты),
  часть 6 (шторы), затем общая проверочная цепочка. Production не менялся.

- 2026-09-07, новый разрешённый проход продолжается. Миграционная основа
  сохранена в `56c205f`, настройки и координатор кладовки в `a4dd87a`.
  Последний полный pytest: 2610 passed, 4 skipped, 1238 subtests.
  Пакет HACS проверен до и после commit; worktree был чистым при передаче.
  Настройки сохраняются через проверяемое хранилище, controls формирует
  сервер. Сквозной тест кладовки использует настоящий ScenarioService,
  Node-RED async_plan, LightAutomationPriority и executor. Проверены
  выключение своего света через 120 секунд, restart, ручное вмешательство,
  неизвестные датчики и запрет повторной неподтверждённой отправки.
  Вытяжка остаётся без привязки: её ветка пропускается до выбора устройства.
  Все восемь activation_ready=False. Следующие части: общая ручная защита,
  питание, плавная яркость и расписания коридоров, остальные комнаты,
  шторы и климатическая задержка, затем общая проверочная цепочка.
  Tester нового прохода ещё не запускался. Production остаётся 1.52.226.
  План: `docs/superpowers/plans/2026-09-07-consolidation-recovery-implementation.md`.
  Нижние записи описывают прошлые попытки, не результат текущего прохода.

- 2026-09-07, актуальный результат: `ed5f809` не готов к выпуску.
  Независимый разбор подтвердил невозможность создания пяти новых scenarios
  текущей миграцией и искусственный исходный fixture из восьми записей.
  Четыре JS не проходят настоящий `async_plan`; положения штор фильтруются,
  sunrise и manual-open неверны, новые световые правила не подключены
  полностью. План выхода с критериями:
  `docs/superpowers/plans/2026-09-07-consolidation-rescue.md`.
  Admin API исправен: 200; 403 публичного маршрута связан с tablet-only
  ролью. Read-only inventory: 58 сценариев, 46 enabled, три managed.
  Production `1.52.226`, устройства и живые сценарии в этом продолжении
  не менялись. Не считать прежние зелёные fixtures доказательством готовности.
  Последний тестировщик: `PYTHONPATH=. pytest -q`, 2541 passed, 4 skipped,
  1235 subtests passed за 301.89 s. Приёмка BLOCKED; ревью и аудит не начаты.
...
- 2026-09-05: повторная общая агентная цепочка остановлена на этапе worker.
  ...
  сценариев. Профильная сверка корневого координатора проходит: 188 тестов и
  5 подпроверок. Этап остаётся BLOCKED: gate пока только проверяет старое
  состояние и receipt, но не выполняет атомарные ScenarioService/Node-RED CAS
  prepare, update, commit, compensation и rollback. Долговечная блокировка
  тамбура на 180 секунд, передача typed intent и безопасный порядок запуска
  также не завершены. Tester, reviewer и security_auditor не запускались.
  Версия не повышалась, tag, push, release, установка, живое обновление
  сценариев и физические команды не выполнялись. Production остаётся на
  HACS `1.52.214`.

- 2026-09-04: цепочка умных сценариев выключателей остановлена на этапе
  реализации после трёх ограниченных попыток. Исследование и план завершены,
  в ветке `codex/hacs-smart-switches-2026-09-04` сохранены commits `3aa0b88`,
  `754b747`, `85464f3`: исправлены целевые привязки, добавлены типизированные
  намерения и частично подключён адаптер событий устройств Home Assistant.
  Узкая проверка проходит: 153 теста и 5 подпроверок. Реализация не готова к
  выпуску: отсутствуют версионированная CAS-миграция managed-сценариев и
  долговечная защита тамбура до 180 секунд подтверждённого отсутствия с
  безопасным восстановлением после перезапуска и закрытием при ошибке
  хранилища. Тестировщик, финальное ревью, release и установка не запускались.
  Физические команды и тесты на реальных устройствах не выполнялись.

- 2026-09-04: HACS `1.52.214` выпущен и установлен в production для
  восстановления общей целевой температуры. Работающий термостат получает и
  подтверждает цель сразу, выключенные кондиционеры сохраняются как
  отложенные до включения без ложного таймаута. Release commit/tag `370d0e1` /
  `v1.52.214`, GitHub Actions `33907579185` успешен. Gate: 2072 выпускных
  теста, 4 пропуска; pytest 2192 теста, 4 пропуска и 1129 подтестов; 245
  критических тестов, 77% branch coverage; Chromium 43 из 43 и полный обход
  709 из 709 элементов. Backup `b6344afe` сохранён локально и на
  `hassio.KeeneticSSD`. Configuration check до и после установки успешен,
  выполнен один restart, installed/latest равны `v1.52.214`, ConfigEntry
  `loaded`, все 557 файлов совпадают с release. Финальная живая команда 25,5 °C
  завершилась за 0,667 с: один активный прибор подтверждён, пять выключенных
  отложены, ошибок и активных операций нет. Runtime `managed`, свеж, команды
  разрешены. Android `1.0.273` всё ещё отображает правдивый
  `saved_deferred_offline` как ошибку; нужен отдельный private Android release
  после подтверждения нового пользовательского сообщения.

- 2026-09-04: HACS `1.52.213` выпущен и установлен после отдельного исправления
  11 выпускных сбоев. `1.52.211` отозван из-за конфликта собственных маршрутов
  `OPTIONS` со штатным CORS Home Assistant, production сразу возвращался на
  `1.52.210`. `1.52.212` отозван после живой проверки: неполный первый каталог
  оставлял API защиты света на 503 даже после прогрева. Commit/tag `8192d18`,
  GitHub Actions `33878512641` успешен. Локально прошли выпускные 2071 тест и
  4 пропуска, сводные 2191 тест, 4 пропуска и 1129 подтестов, 245 критических
  тестов с 77% покрытия ветвей, Chromium 42 обычных сценария и полный обход
  735 из 735 элементов. Backup `1373edeb` сохранён. До и после установки
  configuration check валиден, выполнен один restart, ConfigEntry `loaded`,
  installed/latest равны `v1.52.213`, все 109 frontend-ресурсов совпадают с
  release. Живой API защиты восстановился с 503 во время первого прогрева до
  200 и остался доступен после готовности каталога. Физические команды не
  отправлялись. Android остаётся `1.0.273` build `277`, отдельный релиз не
  нужен. Отдельный незакрытый риск: после restart климатический runtime снова
  отвечает 503 при загруженной интеграции; capability климата закрыта, ошибок
  Hausman в журнале нет, SSH-нода недоступна. Код climate storage между
  `1.52.210` и `1.52.213` не менялся, поэтому откат не исправит сохранённое
  состояние. Ledger не очищался, climate-команды не отправлялись.
...
- 2026-09-03: финальная живая точка после release `1.52.210`: локальные gates
  ...
  climate API на 503. Откат файлов на `1.52.209` и `1.52.205` не помог, что
  подтверждает несовместимое состояние сохранённого authenticated ledger, а не
  загрузку Python-файлов. Raw main/sidecar не удалялись. Production files
  сейчас `1.52.205`, latest `1.52.210`, ConfigEntry loaded, общая цель
  `25,0°C`, `control_revision=15`, backup `491a65fd`. Нужен явный выбор:
  восстановление текущего журнала либо архивирование точных файлов и новый
  ledger. Вода, автоматы и свет не затрагивались.
...
- Last sync: 2026-09-07T07:27:12+03:00.
<!-- llm-wiki-sync:end -->
