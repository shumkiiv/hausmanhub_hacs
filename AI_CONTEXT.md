...
Last updated: 2026-08-21 (главная HACS локально выровнена по ширине с остальными страницами; production остаётся на 1.52.133).

## Current work

- 2026-08-21 (Codex): изменён только HACS frontend. Главная получила тот же
  предел ширины, центрирование и горизонтальные поля, что обычные страницы:
  `max-width: 1600px`, `margin: 0 auto`, по 34 px слева и справа. Внутренняя
  планшетная сетка и высота экрана не менялись. На viewport 1920 px границы
  `main` у главной и «Освещения» совпали точно: 160-1760 px при раскрытом и
  свёрнутом rail, горизонтального overflow нет. Дополнительно проверены
  light/dark на 1280x800. Профильные тесты: 118 passed, 82 subtests;
  отдельный контракт главной: 31 passed, 50 subtests. Версия остаётся
  `1.52.133`; release, push и deploy не выполнялись.
