...
## Подготовлен repair ошибочной записи climate runtime, 2026-09-09

- В `ClimateTabletService` добавлена узкая authenticated-миграция: только
  надёжная общая цель с корректной квитанцией и checkpoint, ошибочно
  сохранённая как `accepted_timeout`, переводится в `terminal_mixed`.
  Запрос, квитанция, счётчики, ручное владение, область действия и запрет
  повтора не меняются. Исполнитель климатических приборов не вызывается.
- Новый regression-тест воспроизводит подтверждённую, ручную и истёкшую
  ветви. Он сначала получил fail-closed отказ загрузки, затем подтвердил
  миграцию, duplicate replay и отсутствие новых вызовов устройств.
- Профильные тесты: 151 PASS, 33 subtests PASS. Полный pytest: 2881 PASS,
  4 skipped, 1293 subtests PASS и один прежний независимый FAIL в
  `test_external_contract_pin`: vendored `climate-runtime.schema.json`
  имеет hash `80a16f...cf07b`, а тест для pin 0.65.1 ожидает
  `964fa9...baa3`. Код repair и контракт не менялись этой задачей.
  Публикация, установка, restart и изменение живого хранилища не выполнялись.
