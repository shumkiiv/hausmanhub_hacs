...
- 2026-08-27: подготовлен HACS `1.52.182` для исправления съехавшей иконки в
  бейдже `Hausman · конструктор` и `Node-RED · код`. В `1.52.181` CSS уменьшал
  только внешнюю коробку `ha-icon` до 15 px, а Home Assistant продолжал
  рисовать внутреннюю Material-иконку штатным размером 24 px. Она выходила на
  рамку и подпись. Теперь заданы `--mdc-icon-size:15px`, блочное отображение,
  нулевая line-height и безопасное ограничение переполнения. Геометрический
  browser-тест проверяет поля 7 px, промежуток 5 px и вертикальный центр.
