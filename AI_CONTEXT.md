...
Last updated: 2026-08-21 (главная и киоск HACS повторяют компоновку планшета; версия пока 1.52.132).

## Current work

- 2026-08-21 (Codex): только HACS frontend главной и режима киоска
  перенесён на актуальную планшетную компоновку. Главная использует те же
  пять рядов, крупный Hero с навигацией комнат, три карточки погоды, цели и
  комфорта, четыре избранных сценария, нижние панели энергии и света.
  Правая колонка повторяет два планшетных режима: компактный при раскрытом
  rail и подробный при свёрнутом. Киоск повторяет планшетные пропорции
  Hero, трёх метрик, сценариев, погоды, состояния дома и домофона. Белые
  кнопки меню сохранены по предыдущему решению владельца. Используются
  только реальные данные HACS, отсутствующие системные показатели не
  подменяются демонстрационными. Визуально проверены light/dark, 1280x800
  и 1600x1000, раскрытый и свёрнутый rail, обычный режим и киоск: ошибок и
  переполнения нет. Full `python3 -m pytest -q`: 1548 passed, 4 skipped,
  984 subtests. Backend, API, Android, storage и production не менялись.
