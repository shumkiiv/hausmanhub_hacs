...
- 2026-08-22 (Codex): изменён только HACS frontend страницы «Сценарии».
  ...
  фильтры по способу запуска, фильтр по комнатам и группы «Режимы дома»,
  «Климат», «Медиа», «Шторы». Системные сценарии скрыты по умолчанию и
  доступны только через отдельный фильтр. Их технические названия, группа и
  описания показаны по-русски, но неизменённые значения возвращаются в
  серверный payload без подмены источника истины. Редактор использует точную
  планшетную структуру: шесть состояний готовности, три колонки с весами
  0.80/1.10/1.12, сворачиваемые правила и общий footer. На узком экране
  колонки последовательно прокручиваются и не накладываются друг на друга.
  Browser QA: 1440x900 light/dark, 1024x900 и 900x900 light, список и
  редактор без наложений. Профиль сценариев: 7 passed; расширенный frontend:
  185 passed; полный staged local release gate: 1621 тест, 4 skipped. Feature
  commit `f50ab8f`, подготовлена версия `1.52.149`; release, push и deploy не
  выполнялись. Android, backend, API, contracts, storage и исполнение
  сценариев не менялись.

- 2026-08-22 (Codex): изменён только HACS frontend раздела «Энергия».
  Устранены две причины раздутой компоновки: пустая колонка 275 px рядом с
  графиком и CSS-конфликт, который превращал строку счётчика в вертикальный
  блок. График теперь использует всю ширину, адаптивный canvas без
  растягивания, ровную шкалу Вт/кВт, пять временных отметок с датами на
  границах суток и часовой пояс Home Assistant. Добавлены средняя линия,
  точная подсказка по наведению и касанию, управление с клавиатуры и
  честная подпись «Последний час». Карточка счётчика показывает механические
  цифры, дату показания, расход текущего цикла и следующую передачу без
  пустого места и голых прочерков. Проверены 1504x1200 light/dark,
  1100x1050 light, интерактивная подсказка и отсутствие горизонтального
  overflow. Профиль frontend: 34 passed и 50 subtests; расширенный профиль:
  166 passed и 50 subtests; полный local release gate: 1620 тестов, 4
  skipped. Feature commit `4d6b2a9`. Версия остаётся `1.52.148`; release,
...
- 2026-08-22 (Codex): изменён только HACS frontend раздела «Климат».
  ...
  и регуляторы не сжимаются. В заголовке показана текущая температура,
  влажность и цели разнесены по отдельным строкам, а в регуляторах видны
  полные названия и значения целей. Для режима только просмотра убраны
  неактивные кнопки и повторяющаяся подпись «Недоступно». Проверены
  семантические иконки помещений; общий значок здания заменён на понятный
  дом. Профиль frontend: 106 passed, 50 subtests; JS syntax и
  `git diff --check` зелёные. Feature commit `a8e1cd8`. Версия остаётся
  `1.52.148`; release, push и deploy не выполнялись. Android, backend, API,
  contracts и storage не менялись.

- 2026-08-22 (Codex): изменён только HACS frontend. Во время выполнения
  команды сверху появляется синяя плашка «Команда отправляется» с понятным
  названием действия, а нажатая кнопка получает локальный вращающийся
  индикатор, синюю рамку и `aria-busy`. Состояние удерживается до ответа,
  повторные действия остаются заблокированы существующим busy-механизмом,
  после завершения индикаторы снимаются и показывается прежний результат или
  ошибка. Кнопки, range, checkbox и select покрыты одним механизмом. Профиль
  frontend: 105 passed, 50 subtests; JS syntax и `git diff --check` зелёные.
  Feature commit `0a27687`. Версия остаётся `1.52.148`; release, push и
  deploy не выполнялись. Android, backend, API, contracts и storage не
  менялись.
...
- 2026-08-22 (Codex): только HACS frontend перестал выводить технические
  ...
  `scenario_failed` и `restarted_by_new_trigger`, получают понятные русские
  подписи. Неизвестный системный код заменяется безопасным русским статусом,
  поэтому новый внутренний идентификатор не попадёт пользователю. Карточка
  «Дом сейчас» открывает «Комнаты», «Последняя активность» открывает
  «Сценарии»; работают мышь, Enter и пробел, добавлены hover и focus
  состояния. Профиль frontend: 141 passed, 50 subtests. Версия остаётся
  `1.52.148`; release, push и deploy не выполнялись. Android, backend, API,
  contracts и storage не менялись.

- 2026-08-22 (Codex): только HACS frontend устранил наложение раскрытых
  панелей «Показания энергии» и «Освещение» на соседние карточки при узкой
  desktop-компоновке. В диапазоне container 1050-1219 px подробная энергия
  переходит на две колонки метрик и двухстрочные источники, освещение
  переносит состояние под название, заголовки могут безопасно переноситься.
  Обе карточки остаются в одной линии. Карточки ограничивают видимое
  содержимое своими границами, внутренние списки прокручиваются. До
  исправления на viewport 1100 px энергия заходила в освещение примерно на
  31 px; после исправления между содержимым и соседней карточкой остаётся
  19 px, вертикальный gap до событий 8 px, нижние границы overview и правой
  колонки совпадают. Browser QA: 1504x1146 light/dark, 1280x900 light,
  1100x900 light/dark, runtime errors и горизонтальный overflow отсутствуют.
  Профиль UI: 102 passed, 50 subtests. Версия остаётся `1.52.148`; release,
  push и deploy не выполнялись. Android, backend, API, contracts и storage
  не менялись.
