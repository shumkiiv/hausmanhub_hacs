...
- 2026-09-03: живая `1.52.209` устранила общий отказ: возврат к `25,0°C`
  создал pending с тремя подтверждёнными 1/1, одним ожидающим 1/1, одним
  manual 0/0 и одним deferred 0/0. Опрос через 30 секунд выявил последний
  дефект: manual/deferred не сохранялись как terminal и превращались в
  `accepted_timeout` 1/1. Кандидат `1.52.210` сохраняет статусы manual и
  deferred при polling; RED/GREEN и смешанные профильные тесты прошли.
  Production пока `1.52.209`, цель `25,0°C`, `control_revision=15`.

- 2026-09-03: после первого исправления пустой комнаты дополнительный RED
  воспроизвёл вторую часть production-сбоя: сокращённый contour передавался
  движку со старым room scope и exact target facts, что давало
  `application plan facts are inconsistent`. Финальный кандидат `1.52.209`
  синхронизирует все три представления с комнатами, где остался исполнитель
  выбранной оси. Профильные 24 теста и 34 подпроверки прошли. Production пока
  `1.52.207`, цель `25,0°C`, `control_revision=13`.

- 2026-09-03: `1.52.207` локализовал живую ошибку общей цели до
  `ContourViolation: contour room needs at least one device` в повторной
  температурной проверке. После исключения ручного владельца его комната
  оставалась пустой, из-за чего уже принятые команды остальных комнат
  превращались в общий 1/0 отказ. Подготовлен корневой hotfix `1.52.208`:
  повторный план отбрасывает комнату без исполнителя выбранной оси, сохраняя
  исходную ручную или deferred квитанцию. Тест прошёл RED/GREEN. Production
  пока `1.52.207`, цель возвращена на `25,0°C`, `control_revision=13`.
