...
- 2026-09-04: commit `3cd1618` завершил изолированный ремонт защиты после
  ручного выключения света и перевёл полный browser gate на явные назначения
  `command`, `ui-only`, `blocked`. Финальный локальный отчёт охватывает 735 из
  735 видимых элементов в 16 состояниях: 708 безопасно нажаты, 27 оставлены
  нетронутыми, все списки ошибок пусты, внешняя сеть не использовалась.
  Профильные проверки: 165 passed и 203 subtests, browser защиты света 8 из 8,
  настройки панели 76 из 76. Общий pytest: 2171 passed, 4 skipped, 11 fail;
  те же 11 fail воспроизводятся на исходном `58a3b63` и не являются регрессией
  этого commit. Merge, push, release, deploy и физические команды не выполнялись.
