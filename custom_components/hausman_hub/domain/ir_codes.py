...
IR_CODE_SOURCE_PRIORITY = {
    IRCodeSource.SMARTIR: 3,
    IRCodeSource.BROADLINK: 2,
    IRCodeSource.MANUAL: 1,
}


def ir_code_source_priority(source: IRCodeSource) -> int:
    """Return the fixed source precedence used for one command replacement."""

    return IR_CODE_SOURCE_PRIORITY[source]


def required_ir_command_key(
    kind: ClimateDeviceKind,
    action: ClimateFinalDeviceAction,
    target_temperature: float | None,
) -> str | None:
    """Map one planned device action to its exact machine-stable IR command."""

    if kind is ClimateDeviceKind.HUMIDIFIER:
        if action is ClimateFinalDeviceAction.HUMIDIFY:
            return "humidifier.on"
        if action in {ClimateFinalDeviceAction.OFF, ClimateFinalDeviceAction.SAFE_OFF}:
    ...
    if action in {ClimateFinalDeviceAction.OFF, ClimateFinalDeviceAction.SAFE_OFF}:
        return "ac.off"
    if action not in {ClimateFinalDeviceAction.COOL, ClimateFinalDeviceAction.HEAT}:
        return None
    if type(target_temperature) not in {float, int}:
        return None
    mode = "cool" if action is ClimateFinalDeviceAction.COOL else "heat"
    temperature_key = f"{target_temperature:.1f}".replace(".", "_")
    return f"ac.{mode}.{temperature_key}"
