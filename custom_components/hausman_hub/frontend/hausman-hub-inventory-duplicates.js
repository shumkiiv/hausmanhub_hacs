...
export function dupGuide(detail, el, device, groupSize) {
  ...
  guidance.appendChild(el("p", "muted", device.canonical
    ? `HausmanHub выбрал её основной среди ${groupSize} похожих записей по доступности, состоянию и полноте сущностей.`
    : "Удаляйте копию только если ниже указано «Не используется настройками HausmanHub», а все нужные сущности есть у основной записи."));
