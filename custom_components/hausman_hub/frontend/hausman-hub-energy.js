import { renderEnergyHistoryChart } from "./hausman-hub-energy-chart.js?v=1.52.230";
import { createLibraryHero } from "./hausman-hub-library-hero.js?v=1.52.230";
import { energySettingsDraft, energySettingsSaveDisabled, loadEnergyMeter, meterConfigured, meterNumber, renderEnergyAnomalyFields, renderEnergyMeterCard } from "./hausman-hub-energy-meter.js?v=1.52.230";
import { enhanceAppendedModal } from "./hausman-hub-modal.js?v=1.52.230";
import { mergeEnergyHistoryResponses, splitEnergyWindows } from "./hausman-hub-pagination.js?v=1.52.230";
import { breakerConfirmation, configuredElectricalBreaker } from "./hausman-hub-device-actions.js?v=1.52.230";
...
function compactEnergySettings(panel, container, energy, deps) {
  const { el, setAttr } = deps;
  const draft = panel._energyDraft || energySettingsDraft(energy);
  ...
  const refreshSaveState = () => {
    save.disabled = energySettingsSaveDisabled(draft, panel._energySettingsSaving);
  };
