...
def validate_managed_source(
    scenario_id: str, source: object
) -> tuple[dict[str, object], ...]:
    """Validate one editable function without executing or deploying it."""

    if (
        isinstance(source, Mapping)
        and source.get("scenarioId") == "system-tambur-adaptive-controller"
        and (
            source.get("topology") == "tambur-decision-eight-node-v1"
            or isinstance(source.get("nodes"), list)
        )
    ):
        raise NodeRedSourceInvalid(
            "Полный комплект решения тамбура обновляется только внутренним API.",
            code="decision_bundle_requires_internal_api",
        )
...
class NodeRedScenarioBackend:
    ...
    async def _async_tambur_decision_snapshot(self) -> tuple[str, str]:
        """Read and verify both global and per-flow Tambur graph views."""

        from .scenario_node_red_decision import (  # noqa: PLC0415
            prepare_tambur_decision_bundle,
            verify_tambur_decision_flow,
            verify_tambur_decision_global_nodes,
        )

        bundle = prepare_tambur_decision_bundle()
        revision, global_nodes = await self._async_global_snapshot()
        try:
            global_hash = verify_tambur_decision_global_nodes(
                global_nodes, bundle
            )
        except ValueError as error:
            raise NodeRedBackendError(
                "Node-RED Tambur decision global graph is untrusted"
            ) from error
        status, flow = await self._raw_request(
            "GET", f"/flow/{bundle['id']}", ingress=True
        )
        if status != 200 or not isinstance(flow, Mapping):
            raise NodeRedBackendError(
                "Node-RED Tambur decision flow is unavailable"
            )
        try:
            flow_hash = verify_tambur_decision_flow(flow, bundle)
        except ValueError as error:
            raise NodeRedBackendError(
                "Node-RED Tambur decision flow graph is untrusted"
            ) from error
        if flow_hash != global_hash:
            raise NodeRedBackendError(
                "Node-RED Tambur decision graph views do not match"
            )
        return revision, flow_hash

    async def async_prepare_tambur_decision_bundle(self) -> dict[str, object]:
        """Install and verify the exact decision tab using one revision CAS."""
        ...
        bundle = prepare_tambur_decision_bundle()
        revision_before, global_nodes = await self._async_global_snapshot()
        created = False
        returned_revision = revision_before
        try:
            verify_tambur_decision_global_nodes(global_nodes, bundle)
        except ValueError:
            try:
                validate_tambur_decision_install_target(global_nodes)
            except ValueError as error:
                raise NodeRedBackendError(
                    "Node-RED Tambur decision install target conflicts"
                ) from error
            payload = {
                "rev": revision_before,
                "flows": [
                    *global_nodes,
                    *tambur_decision_global_nodes(bundle),
                ],
            }
            status, response = await self._raw_request(
                "POST", "/flows", payload=payload, ingress=True
            )
            data = self._data(response) if isinstance(response, Mapping) else {}
            returned_revision = data.get("rev")
            if (
                status != 200
                or not isinstance(returned_revision, str)
                or not returned_revision
                or returned_revision == revision_before
            ):
                raise NodeRedBackendError(
                    "Node-RED did not confirm the Tambur decision bundle"
                )
            created = True

        revision_after, topology_hash = (
            await self._async_tambur_decision_snapshot()
        )
        if (
            revision_after != returned_revision
            or created and revision_after == revision_before
            or not created and revision_after != revision_before
        ):
            raise NodeRedBackendError(
                "Node-RED Tambur decision bundle revision is inconsistent"
            )
        return {
            "created": created,
            "flowId": bundle["id"],
    ...
    async def async_calculate_tambur_decision(
        self, request: Mapping[str, object]
    ) -> dict[str, object]:
        """Run the internal calculator with exact graph checks on both sides."""

        from .scenario_node_red_decision import (  # noqa: PLC0415
            TAMBUR_DECISION_ENDPOINT,
        ...
        try:
            ...
            raise NodeRedBackendError(
                "Node-RED Tambur decision input is invalid"
            ) from error
        revision_before, topology_before = (
            await self._async_tambur_decision_snapshot()
        )
        status, response = await self._raw_request(
            "POST", f"/endpoint{TAMBUR_DECISION_ENDPOINT}",
            payload=request, ingress=True
        )
        try:
            revision_after, topology_after = (
                await self._async_tambur_decision_snapshot()
            )
        except NodeRedBackendError as error:
            raise NodeRedBackendError(
                "Node-RED Tambur graph changed during decision"
            ) from error
        if (
        ...
        if status != 200 or not isinstance(response, Mapping):
            raise NodeRedBackendError("Node-RED Tambur decision request failed")
        try:
            validate_tambur_decision(request, response)
        except ValueError as error:
            raise NodeRedBackendError(
                "Node-RED decision response is invalid"
            ) from error
    ...
    async def async_update_source(
        ...
        *,
        ...
        from .scenario_node_red_decision import (  # noqa: PLC0415
            tambur_decision_flow_id,
        )

        if flow_id == tambur_decision_flow_id():
            raise NodeRedSourceInvalid(
                "Полный комплект решения тамбура обновляется только внутренним API.",
                code="decision_bundle_requires_internal_api",
            )
