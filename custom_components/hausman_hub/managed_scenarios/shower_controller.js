...
const actions = [];
const semanticCabinetToggle = trigger.source === 'manual' &&
  trigger.binding === 'shower-cabinet' && trigger.typed_intent === 'toggle' &&
  (trigger.direct_user_intent === 'on' || trigger.direct_user_intent === 'off') &&
...
if (semanticCabinetToggle) {
  ...
  if (cabinet === 'on' || cabinet === 'off') {
    setSwitch(actions, 'cabinet', trigger.direct_user_intent === 'on', true);
  }
...
trace.push({
  ...
  reason: lightBranch === 'light_unknown'
    ...
    : lightBranch === 'cabinet_toggle_unavailable'
      ? 'Состояние подсветки шкафа недоступно, команда не отправляется.'
