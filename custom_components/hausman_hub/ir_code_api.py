...
from .application.ir_code_sources import (
...
from .domain.ir_codes import IRCodeSource
...
class _IrCodeView(HomeAssistantView):
    ...
    def _service_ready(self) -> IRCodeService | None:
        service = self._hass.data.get(DOMAIN, {}).get("ir_code_service")
        if not isinstance(service, IRCodeService):
            return None
...
class IrCodesScanView(_IrCodeView):
    ...
    async def get(self, request: Any) -> Any:
        ...
        if service is None:
            return self._unavailable()
        try:
            smartir_codes = scan_smartir_device_codes(self._hass)
            broadlink_remotes = read_broadlink_codes(self._hass)
        except Exception:
            return self._unavailable()
        return self.json(
            {
                "smartir": smartir_codes,
                "broadlink_remotes": broadlink_remotes,
            },
            headers=NO_STORE_HEADERS,
        )
...
class IrCodeTestView(_IrCodeView):
    ...
    async def post(self, request: Any) -> Any:
        ...
        if isinstance(code_id, str) and code_id:
            ...
            if not isinstance(remote_entity_id, str) or not remote_entity_id:
                return self.json_message(
                    "code_id or remote_entity_id + code_data are required.",
                    HTTPStatus.BAD_REQUEST,
                    headers=NO_STORE_HEADERS,
                )
...
def ir_code_api_views(
    ...
    return (
        IrCodesView(hass),
        IrCodesScanView(hass),
        IrCodeLearnView(hass),
        IrCodeTestView(hass),
        IrCodeDeleteView(hass),
    )
