...
## 2026-09-04: защита света и явный аудит интерфейса готовы в ветке

- В `codex/hacs-light-protection-repair-2026-09-04` создан commit `3cd1618`.
  Снимок защиты теперь получает серверный остаток минимального времени, строго
  проверяется клиентом и не запускает бесконечное перечитывание при нуле.
- Browser gate больше не опирается на текстовые исключения. Каждый видимый
  доступный элемент получает стабильный технический ключ и одно из назначений:
  точная команда, локальное действие или запрет на нажатие. Неразмеченный
  элемент закрывает проверку ошибкой.
- Финальный прогон: 735 из 735 элементов, 708 нажатий, 27 безопасных запретов,
  ноль пропусков, лишних команд, неподтверждённых эффектов и внешних запросов.
  Профильные Python-проверки дали 165 passed и 203 subtests, экран защиты света
  8 из 8, настройки панели 76 из 76.
- Общий pytest сохранил 11 известных сбоев исходной ветки при 2171 passed и
  4 skipped. Все 11 отдельно воспроизведены на чистом commit `58a3b63`.
- Ветка и worktree сохранены. Merge, push, release и deploy ждут выбора владельца.

## 2026-08-24: HACS 1.52.162 подготовлен к финальному deploy

- Исполнитель сценариев больше не записывает вытесненный restart-запуск как
  cancelled в журнал. Вложенный вызов с причиной restarted_by_new_trigger
  остаётся пропущенным шагом и не делает родительский сценарий ошибочным.
- Если сценарий успел включить устройство и последующий шаг завершился
  ошибкой, исполнитель без ожидания выполняет предусмотренный далее turn_off
  этого же устройства. Квитанция получает признак safety_cleanup.
- API списка сценариев закреплён тестом как полный источник definition для
  планшета: сохраняются порядок всех шагов и выключенные сценарии.
- Версия manifest, README и cache-ссылки frontend синхронизированы на
  `1.52.162`. Full local release gate: 1665 tests, 4 skipped, все fixtures,
  Android compatibility и package safety passed.
- Kimi read-only review был запрошен, но не вернул ответ за контрольное время.
  Его процесс остановлен без изменения файлов. Codex выполнил итоговый review.
