...
- Изменён только HACS frontend. Нижняя лента комнат, стрелки, активная
  комната, точки, меню избранного и кнопка дома используют одну систему
  полупрозрачных поверхностей.
- В светлой теме навигация стала нейтральной и легче визуально. В тёмной
  теме фиксированная серо-бежевая заливка заменена глубоким стеклянным слоем
  с читаемыми иконками, текстом и активным состоянием.
- Размер Hero, порядок комнат, прокрутка и обработчики не менялись. Те же
  theme-aware состояния применены в режиме киоска.
- Browser QA: 1504x900 light/dark, 1100x900 light/dark и kiosk 1504x900
  light/dark. Профиль frontend: 108 тестов; полный staged local release
  gate: 1621 тест, 4 skipped.
- Feature commit `61e0712`, версия `1.52.150` подготовлена. Release, push и
  deploy не выполнялись. Android, backend, API, contracts и storage не
  менялись.

## 2026-08-22: планшетный каталог и редактор сценариев HACS

- Изменён только HACS frontend. Каталог сценариев повторяет планшетные
  карточки высотой 196 px и действия размером 112x40 и 120x40 px.
- Добавлены быстрые фильтры «Ручные», «Автоматика», «Гибридные» и
  «Системные», а также фильтр по комнатам. Системные сценарии скрыты по
  умолчанию.
- Сценарии собраны в группы «Режимы дома», «Климат», «Медиа» и «Шторы».
  Технические названия системных сценариев отображаются по-русски, при этом
  исходные серверные значения остаются источником истины.
- Редактор перенесён в планшетную трёхколоночную структуру с шестью
  состояниями готовности, сворачиваемыми правилами и общим footer. На узком
  экране секции идут последовательно и не перекрываются.
- Browser QA: 1440x900 light/dark, 1024x900 и 900x900 light. Профиль
  сценариев: 7 passed; расширенный frontend: 185 passed; полный staged local
  release gate: 1621 тест, 4 skipped.
- Feature commit `f50ab8f`, версия `1.52.149` подготовлена. Release, push и
  deploy не выполнялись. Android, backend, API, contracts, storage и
  исполнение сценариев не менялись.

## 2026-08-22: информативный график энергии и компактный счётчик HACS

- Изменён только HACS frontend на базе `1.52.148`. Android, backend, API,
  contracts и storage не затрагивались.
- График больше не оставляет пустую колонку справа и не растягивает canvas.
  Шкала получает ровные деления в Вт или кВт, а ось времени показывает до
  пяти отметок, даты на краях периода и время из часового пояса Home
  Assistant.
- Метрики различают последний час, среднее, пик и минимум. Пунктирная линия
  показывает среднее, а наведение, касание и клавиши позволяют увидеть
  точные время и значение выбранной точки.
- Карточка электросчётчика собрана в одну адаптивную строку: механические
  цифры, дата последней передачи, текущий цикл и следующая передача. CSS
  больше не переключает её в вертикальный режим, между панелями есть 12 px.
- Визуально проверены 1504x1200 light/dark и 1100x1050 light. Canvas имеет
  одинаковый CSS и pixel size, runtime errors и горизонтальный overflow
  отсутствуют.
- Профиль frontend: 34 passed и 50 subtests; расширенный профиль: 166
  passed и 50 subtests; полный local release gate: 1620 тестов, 4 skipped.
- Feature commit `4d6b2a9`. Версия остаётся `1.52.148`; release, push и deploy
  не выполнялись.

## 2026-08-22: удобные карточки управления устройствами HACS

- Изменён только HACS frontend на базе `1.52.148`. Android, backend, API,
  contracts и storage не затрагивались.
- Модальная карточка стала компактнее: шапка, фотография и факты занимают
  меньше места, каналы устройства выстроены в адаптивную сетку, а люстра
  показывает яркость и температуру света без визуального масштабирования.
- Для каждого канала выводятся понятное название, текущее состояние и только
  актуальное действие. Повторяющиеся команды вроде одновременных «Включить»,
  «Выключить» и «Переключить» скрыты.
- Администратор может переименовать клавишу или действие прямо в карточке.
  Имя записывается стандартной командой `config/entity_registry/update` в
  реестр сущностей Home Assistant. HACS не хранит локальный дубликат имени.
- Проверены светлая и тёмная темы, узкий экран, JS syntax и
  `git diff --check`. Профиль frontend: 33 passed и 50 subtests; settings:
  2 passed; wizard: 59 passed; полный release gate: 1619 тестов выполнено,
  4 skipped.
- Feature commit `4292d9f`. Версия остаётся `1.52.148`; release, push и deploy
  не выполнялись.

## 2026-08-22: выравнивание панели внимания безопасности HACS

- Изменён только HACS frontend раздела «Безопасность» на базе `1.52.148`.
- Строки в правой панели «Требуют внимания» получили горизонтальный
  внутренний отступ 12 px. Текст больше не прижат к левой границе, а ширина
  строк сохранена через `box-sizing:border-box`.
- Browser QA: 1293x1057, светлая тема, снимки до и после правки. Профиль
  frontend: 106 passed, 50 subtests; `git diff --check` зелёный.
- Feature commit `4c88d90`. Release, push и deploy не выполнялись. Android,
  backend, API, contracts и storage не менялись.

## 2026-08-22: читаемые карточки климата HACS

- Изменён только HACS frontend раздела «Климат» на базе `1.52.148`.
- Карточки комнат повторяют планшетную плотность: две карточки в строке
  вместо трёх. Заголовок показывает текущую температуру, а влажность и цели
  собраны в двух коротких блоках без повторения показаний.
- Регуляторы температуры и влажности получили полные названия и видимые
  значения целей. Если управление недоступно, карточка показывает значение
  без серых кнопок и повторяющегося текста «Недоступно».
- Иконки помещений сверены с их назначением: ванная и душевая, кабинет,
  кладовая, детская. Общий значок здания заменён на понятный значок дома.
- Профиль frontend: 106 passed, 50 subtests; JS syntax и `git diff --check`
  зелёные. Feature commit `a8e1cd8`, версия остаётся `1.52.148`.
